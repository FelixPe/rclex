...
#ifndef ROS_DISTRO_humble
ERL_NIF_TERM nif_tf2_msgs_action_lookup_transform__send_goal__response_type_description(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ignore_unused(argv);
  ...
  const rosidl_runtime_c__type_description__TypeDescription *description =
      tf2_msgs__action__LookupTransform_SendGoal_Response__get_type_description(ts_p);
  if (description == NULL) return raise(env, __FILE__, __LINE__);

  return make_type_description_term(env, description);
}

ERL_NIF_TERM nif_tf2_msgs_action_lookup_transform__send_goal__response_type_description_sources(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ignore_unused(argv);
  ...
  const rosidl_runtime_c__type_description__TypeSource__Sequence *sources =
      tf2_msgs__action__LookupTransform_SendGoal_Response__get_type_description_sources(ts_p);
  if (sources == NULL) return raise(env, __FILE__, __LINE__);

  return make_type_sources_term(env, sources);
}

ERL_NIF_TERM nif_tf2_msgs_action_lookup_transform__send_goal__response_type_hash(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ignore_unused(argv);

  if (argc != 0) return enif_make_badarg(env);

  const rosidl_message_type_support_t *ts_p = ROSIDL_GET_MSG_TYPE_SUPPORT(tf2_msgs, action, LookupTransform_SendGoal_Response);
  const rosidl_type_hash_t *hash = tf2_msgs__action__LookupTransform_SendGoal_Response__get_type_hash(ts_p);
