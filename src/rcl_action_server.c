...
ERL_NIF_TERM nif_rcl_action_accept_new_goal(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  rcl_action_goal_info_t **goal_info_pp;
  if (!enif_get_resource(env, argv[1], rt_ros_message, (void **)&goal_info_pp))
    return enif_make_badarg(env);

  rcl_action_goal_handle_t *goal_handle_p =
      rcl_action_accept_new_goal(action_server_p, *goal_info_pp);
...
ERL_NIF_TERM nif_rcl_action_server_goal_exists(ErlNifEnv *env, int argc,
  ...
  bool goal_exists;
  goal_exists = rcl_action_server_goal_exists(action_server_p, *goal_info_pp);
